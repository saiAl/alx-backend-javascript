/**
 * The function `countStudents` processes a CSV file of student
 *    data and logs the total number of students,
 * as well as the number of students in each field of study,
 ...
 * 2. If the file cannot be read (e.g., it doesn't exist),
 *    it throws an error with the message "Cannot load the database".
 *
 * 3. If the file is successfully read, it:
 *    - Splits the content into an array of lines (rows),
 *        where each line corresponds to a student record.
 *    - Extracts the CSV header to identify fields (e.g., first name, last name, etc.).
 *    - Ignores the header row and counts the number of valid
 *        student records (non-empty lines).
 *    - Identifies unique fields of study (e.g., CS, SWE)
 *        and logs the total number of students in each field.
 *    - Logs the names of students in each field in the format:
 *        `Number of students in FIELD: X. List: [list of names]`.
  ...
  fs.readFile(path, 'utf8', (error, data) => {
    if (error) {
      ...
      for (const field of fields) {
        ...
        console.log(
          `Number of students in ${field}: ${fieldCount}. List:${arr}`
